...
from nlp_lab.evaluation.labeling.system.base import ISideLabelingSystem
...
class IndevidualSpeakerDebatesObserver(IDebatesObserver):
    def __init__(self, features_extractors, side_labeling_system,
                 alternative_side_labeling_systems=None, debug_print=False):

        self.__features_extractors__ = features_extractors
        self.__labeling_system__ = side_labeling_system
        self.__alternative_labeling_systems__ = alternative_side_labeling_systems
        self.__data__ = []
        self.__labels__ = []
        self.__names__ = []
        ...
        for labeling_system in alternative_side_labeling_systems[:] + [side_labeling_system]:
            if not isinstance(labeling_system, ISideLabelingSystem):
                raise Exception('%s supports only %s derived labeling systems' %
                                (self.__class__.__name__, ISideLabelingSystem.__class__.__name__))
    ...
    def __observe_job__(self, debate, debate_name):
        ...
        for_label, against_label = self.__labeling_system__.create_labels(debate)
        alternative_labels = [als.create_labels(debate) for als in self.__alternative_labeling_systems__]
